...
using Smarticube.API.HKJC.Football.Data;
using Smarticube.API.HKJC.Football.Models;
namespace Smarticube.API.Controllers
{
    [ApiController]
    [Route("api/[controller]/[action]")]
    public class HkjcFootballController : Controller
    {
        private readonly IHkjcFootballRepository _hkjcfootballRepository;
        public HkjcFootballController(IHkjcFootballRepository hkjcfootballRepository)
        {
            _hkjcfootballRepository = hkjcfootballRepository;            
        }

        [HttpGet]
        [ProducesResponseType(200, Type= typeof(IEnumerable<HkjcDataPoolResult>))]
        public async Task<IActionResult> GetResults()
        {
            IEnumerable<HkjcDataPoolResult> footballResults = await _hkjcfootballRepository.GetResults();

            if (!ModelState.IsValid)
                return BadRequest(ModelState);

            return Ok(footballResults);
        }


        [HttpPost]
        [ProducesResponseType(200, Type = typeof(string))]
        public IActionResult ImportHkjcFootballRecordsWsByString([FromQuery] string csvObjs)
        {
